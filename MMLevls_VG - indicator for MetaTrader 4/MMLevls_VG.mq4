...
//---- indicators
   if(MMPeriod>0)
      NewPeriod  =P*(int)MathCeil(MMPeriod/Period());
   else NewPeriod=P;
//----
...
//---- TODO: add your code here
   Comment(" ");
     ...
     {
      buff_str="mml"+(string)i;
      ObjectDelete(buff_str);
      buff_str="mml_txt"+(string)i;
      ObjectDelete(buff_str);
     }
//----
   return(0);
...
//----
      ...
      CurPeriod= Period();
//----
      string _buff_str="LR_LatestCulcBar";
        if(ObjectFind(_buff_str)==-1) 
        {
         ObjectCreate(_buff_str, OBJ_ARROW,0, Time[StepBack], Low[StepBack]-2*Point );
         ObjectSet(_buff_str, OBJPROP_ARROWCODE, MarkNumber);
         ObjectSet(_buff_str, OBJPROP_COLOR, MarkColor);
        }
        else 
        {
         ObjectMove(_buff_str, 0, Time[StepBack], Low[StepBack]-2*Point );
        }
     }
//---- End Of Program
   return(0);
